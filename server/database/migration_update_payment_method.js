// Migration script to update payment_method constraint to remove 'card'
const { db } = require('./db');

async function migrate() {
  try {
    console.log('Running migration: Update payment method constraint...');
    
    // Check if orders table exists
    const tableInfo = await db.promisify.all("PRAGMA table_info(orders)");
    const paymentMethodColumn = tableInfo.find(col => col.name === 'payment_method');
    
    if (!paymentMethodColumn) {
      console.log('Payment method column not found, skipping migration.');
      process.exit(0);
    }
    ...
    // Convert any 'card' payments to 'cash' (or you could set to NULL)
    ...
    if (rowCount.count > 0) {
      await db.promisify.run(`
        ...
        CASE 
          WHEN payment_method = 'card' THEN 'cash'
          ELSE payment_method
        END as payment_method,
        total_amount, created_at, completed_at, paid_at
        FROM orders
      `);
      console.log(`Migrated ${rowCount.count} orders to new table.`);
      console.log('Note: Any orders with "card" payment method were converted to "cash".');
    }
    ...
    console.log('New valid payment methods: cash, mpesa');
    process.exit(0);
  } catch (error) {
